 ...
 */
function providesServices(ctx, pluginName) {
    ...
    return Object.values(ctx.reflect.store).some((impl) => impl.fiber === fiber);
}
function reloadVerdict(ctx, pluginName, resolved) {
    if (pluginName === name) {
...
export async function reloadPlugin(ctx, pluginName, config = {}) {
...
export async function startWatch(ctx, config) {
    ...
    const state = { started: false, scopes: [], watcherReady: false, events: 0, reloads: 0, skipped: 0 };
    if (loader === undefined) {
    ...
    for (const entry of [...loader.entries()]) {
        ...
        if (!pluginName.startsWith('@') || pluginName === name)
            continue;
        const slash = pluginName.indexOf('/');
        const scope = pluginName.slice(0, slash);
        const pkg = pluginName.slice(slash + 1);
        if (pkg === '' || !resolved.watchRoots.includes(scope))
            continue;
        const pkgs = byScope.get(scope) ?? new Set();
        pkgs.add(pkg);
            ...
            try {
                ...
                if (target.pkgSig !== undefined && target.pkgSig !== pkgSig) {
                    ...
                    if (previous !== undefined && previous !== next) {
                        ctx.logger.warn(`[plugin-reloader] "${target.pluginName}" changed its dependency tree; `
...
export function apply(ctx, config) {
    ...
    if (resolved.watchEnabled) {
        ctx.effect(() => {
            let dispose;
            void startWatch(ctx, config).then((handle) => {
