...
export async function startWatch(ctx, config) {
    ...
    const state = { started: false, scopes: [], watcherReady: false, events: 0, reloads: 0 };
    if (loader === undefined || loader.internal === undefined) {
    ...
    for (const [scope, pkgs] of byScope) {
        ...
        watcher.on('error', (error) => {
            if (state.watcherError === undefined)
                state.watcherError = renderError(error);
        });
        watcher.on('all', (event, path) => {
            ...
            if (/^lib[\\/]/.test(restPath) || /^src[\\/]/.test(restPath))
...
export function apply(ctx, config) {
    const resolved = resolveConfig(config);
    const watchState = { started: false, scopes: [], watcherReady: false, events: 0, reloads: 0 };
